...
def get_response(prompt):
    ...
    if any(k in p for k in ["calendario", "disponible", "turno", "reserva", "agenda", "ver", "mostrar"]):
        return {
            "content": "SHOW_CALENDAR",  # Flag especial
...
for i, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        # Manejo especial para calendario
        if msg.get("content") == "SHOW_CALENDAR":
...
👇 **Días más solicitados**""")
        ...
        elif msg.get("content") and "TIME_SLOTS:" in msg.get("content", ""):
            fecha = msg["content"].split("TIME_SLOTS:")[1].strip()
            time_html = get_time_slots_visual(fecha)
            st.markdown(f"✅ **¡Perfecto! Elegiste {fecha}**", unsafe_allow_html=True)
